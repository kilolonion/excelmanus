...
from excelmanus.config import ConfigError, format_deprecated_model_message, get_deprecated_model_replacement, load_config, load_cors_allow_origins
...
class TestDotEnvPriority:
    ...
    def test_dotenv_base_url_loaded(self, monkeypatch, tmp_path) -> None:
        ...
        env_file = tmp_path / '.env'
        env_file.write_text('EXCELMANUS_API_KEY=test-key\nEXCELMANUS_BASE_URL=https://custom.api.com/v1\nEXCELMANUS_MODEL=test-model\n')
        monkeypatch.chdir(tmp_path)
        cfg = load_config()
...
class TestCorsConfig:
    ...
    def test_env_overrides_dotenv_for_cors(self, monkeypatch, tmp_path) -> None:
        env_file = tmp_path / '.env'
        env_file.write_text('EXCELMANUS_CORS_ALLOW_ORIGINS=http://dotenv.com\n', encoding='utf-8')
        monkeypatch.chdir(tmp_path)
...
class TestModelsRouterHooksAndMcpConfig:
    ...
    def test_models_parsed_and_inherit_defaults(self, monkeypatch) -> None:
        self._set_required_env(monkeypatch)
        monkeypatch.setenv('EXCELMANUS_MODELS', '[{"name":"alt","model":"gpt-4o-mini","description":"备用"}]')
        cfg = load_config()

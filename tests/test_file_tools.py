...
class TestListDirectory:
    ...
    def test_flat_hidden_files_shown(self, workspace: Path) -> None:
        result = _payload(file_tools.list_directory(show_hidden=True, depth=0))
        names = [e["name"] for e in result["entries"]]
        assert ".hidden" in names

    def test_flat_subdirectory(self, workspace: Path) -> None:
        result = _payload(file_tools.list_directory("subdir", depth=0))
    ...
    def test_pagination(self, workspace: Path) -> None:
        full = _payload(file_tools.list_directory(depth=0))
        page = _payload(file_tools.list_directory(offset=0, limit=2, depth=0))
    ...
    def test_cursor_pagination(self, workspace: Path) -> None:
        full = _payload(file_tools.list_directory(depth=0, use_default_excludes=False))
        page1 = _payload(file_tools.list_directory(depth=0, limit=2, cursor="0", use_default_excludes=False))
    ...
    def test_disable_default_excludes(self, workspace: Path) -> None:
        (workspace / "outputs").mkdir()
        result = _payload(file_tools.list_directory(depth=0, use_default_excludes=False))
        names = [e["name"] for e in result["entries"]]
    ...
    def test_overview_mode(self, workspace: Path) -> None:
        ...
        result = _payload(file_tools.list_directory(mode="overview", limit=2))
    ...
    def test_tree_depth_1(self, workspace: Path) -> None:
        ...
        result = _payload(file_tools.list_directory(depth=1))
    ...
    def test_tree_hidden_shown(self, workspace: Path) -> None:
        result = _payload(file_tools.list_directory(show_hidden=True, depth=1))
        names = [e["name"] for e in result["tree"]]
        assert ".hidden" in names
...
class TestGetFileInfo:
    def test_file_info(self, workspace: Path) -> None:
        result = _payload(file_tools.get_file_info("hello.txt"))
    ...
    def test_directory_info(self, workspace: Path) -> None:
        result = _payload(file_tools.get_file_info("subdir"))
...
class TestFindFiles:
    def test_search_txt(self, workspace: Path) -> None:
        result = _payload(file_tools.find_files("*.txt"))
        assert result["total"] >= 1
...
class TestCopyFile:
    def test_copy_success(self, workspace: Path) -> None:
        result = _payload(file_tools.copy_file("hello.txt", "hello_copy.txt"))
    ...
    def test_copy_to_subdir(self, workspace: Path) -> None:
        result = _payload(file_tools.copy_file("hello.txt", "newdir/hello.txt"))
        assert result["status"] == "success"
...
class TestGetTools:
    def test_tool_count(self) -> None:
        ...
        assert len(tools) == 6  # read_text_file 恢复 + offer_download

...
def _isolate_attachment_store(tmp_path, monkeypatch):
    ...
    yield
    reset_attachment_store()
_FULL_SPEC = {'version': '2', 'name': 'replica', 'locale': 'zh-CN', 'default_font': {'name': '等线', 'size': 11}, 'sheets': [{'name': 'Sheet1', 'dimensions': {'rows': 3, 'cols': 3}, 'freeze_panes': 'A2', 'cells': [{'address': 'A1', 'value': '产品', 'value_type': 'string', 'style_id': 'header', 'confidence': 0.98}, {'address': 'B1', 'value': '数量', 'value_type': 'string', 'style_id': 'header', 'confidence': 0.98}, {'address': 'C1', 'value': '金额', 'value_type': 'string', 'style_id': 'header', 'confidence': 0.98}, {'address': 'A2', 'value': '苹果', 'value_type': 'string', 'confidence': 1.0}, {'address': 'B2', 'value': 100, 'value_type': 'number', 'confidence': 1.0}, {'address': 'C2', 'value': 500.5, 'value_type': 'number', 'confidence': 0.95}, {'address': 'A3', 'value': '香蕉', 'value_type': 'string', 'confidence': 1.0}, {'address': 'B3', 'value': 200, 'value_type': 'number', 'confidence': 1.0}, {'address': 'C3', 'value': 300, 'value_type': 'number', 'confidence': 0.9}], 'merged_ranges': [], 'styles': {'header': {'font': {'bold': True, 'size': 12, 'color': '#FFFFFF', 'name': '微软雅黑'}, 'fill': {'type': 'solid', 'color': '#4472C4'}, 'alignment': {'horizontal': 'center'}}}, 'column_widths': [15, 10, 12], 'row_heights': {'1': 28}}], 'uncertainties': [{'location': 'C2', 'reason': '小数部分模糊', 'candidate_values': ['500.5', '500.8'], 'confidence': 0.65}]}
...
class TestImageToExcelPipeline:
    ...
    def test_spec_roundtrip(self, tmp_path: Path) -> None:
        ...
        data, summary = create_document_bytes(_FULL_SPEC)
        excel_path.write_bytes(data)
        assert excel_path.exists()
        assert summary['applied'].count('write') == 9
    ...
    def test_rebuild_with_merged_cells_and_styles(self, tmp_path: Path) -> None:
        ...
        from openpyxl import load_workbook
        from tests.workbook_support import create_document_bytes
        merge_spec = {**_FULL_SPEC, 'sheets': [{'name': 'Sheet1', 'dimensions': {'rows': 3, 'cols': 3}, 'freeze_panes': 'A2', 'cells': [{'address': 'A1', 'value': '标题', 'value_type': 'string', 'style_id': 'header', 'confidence': 0.98}, {'address': 'A2', 'value': '苹果', 'value_type': 'string', 'confidence': 1.0}, {'address': 'B2', 'value': 100, 'value_type': 'number', 'confidence': 1.0}, {'address': 'C2', 'value': 500.5, 'value_type': 'number', 'confidence': 0.95}], 'merged_ranges': [{'range': 'A1:C1', 'confidence': 0.95}], 'styles': _FULL_SPEC['sheets'][0]['styles'], 'column_widths': [15, 10, 12], 'row_heights': {'1': 28}}]}
        spec_path = tmp_path / 'spec.json'
        spec_path.write_text(json.dumps(merge_spec), encoding='utf-8')
        excel_path = tmp_path / 'complex.xlsx'
        data, summary = create_document_bytes(merge_spec)
        excel_path.write_bytes(data)
    ...
    def test_workbook_spec_pydantic_roundtrip(self) -> None:
        """WorkbookSpec JSON 序列化/反序列化一致性。"""
        from excelmanus.workbook.spec import WorkbookSpec
        spec = WorkbookSpec.model_validate(_FULL_SPEC)
        json_str = spec.model_dump_json()
        spec2 = WorkbookSpec.model_validate_json(json_str)
    ...
    def test_multimodal_memory_integration(self, tmp_path, monkeypatch) -> None:
        ...
        monkeypatch.setenv('EXCELMANUS_HOME', str(tmp_path))
        reset_attachment_store()
        config = ExcelManusConfig(api_key='test', base_url='https://test.example.com/v1', model='test')
    ...
    def test_provider_image_conversion(self) -> None:
        ...
        messages = [{'role': 'user', 'content': [{'type': 'text', 'text': '分析图片'}, {'type': 'image_url', 'image_url': {'url': 'data:image/png;base64,abc123'}}]}]

...
主循环外无额外 LLM、权限拒绝 / 取消 / 审批等待仍有效。
回合在模型不再调用工具时结束；没有结束工具，也没有轮次预算。
...
class TestPlainTextEndsRound:
    ...
    async def test_content_shape_does_not_block_or_rerun(self, text: str) -> None:
        ...
        result = await run_tool_loop(engine, _make_route_result(), on_event=None)
...
class TestFinishTaskRemoved:
    def test_finish_task_absent_from_catalog(self) -> None:
        engine = _make_engine()
        ...
        assert "finish_task" not in names_v5
...
class TestRealToolLoopFakeLlm:
    ...
    @pytest.mark.asyncio
    async def test_add_numbers_then_text_close(self) -> None:
        engine = _make_engine(max_iterations=5)
        ...
        create_mock = AsyncMock(
            side_effect=[
                ...
                _make_text_response("只算了加法。未写回工作簿，未生成图表。"),
            ]
        )
        engine._client.chat.completions.create = create_mock
...
class TestHardBoundariesRemain:
    ...
    async def test_cancelled_error_propagates(self) -> None:
        ...
        with pytest.raises(asyncio.CancelledError):
            await run_tool_loop(engine, _make_route_result(), on_event=None)
    ...
    async def test_pending_approval_does_not_block_chat(self) -> None:
        ...
        result = await engine.followup("继续处理")
...
class TestRunCodeWritePropagation:
    ...
    @pytest.mark.asyncio
    async def test_run_code_published_paths(self) -> None:
        engine = _make_engine(code_policy_enabled=True)
        engine._has_write_tool_call = False
...
class TestRegistryRefreshOnRecordedWrite:
    ...
    async def test_registry_refresh_triggered_by_record_write_action(self) -> None:
        ...
        engine._client.chat.completions.create = AsyncMock(
            side_effect=[first, _make_text_response("已同步")],
        )

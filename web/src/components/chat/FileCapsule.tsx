...
export function RelatedFilesCard({
  ...
  return (
    <div className="relative mt-2 rounded-2xl border border-[var(--em-hairline)] bg-background">
      {tooltip && (
        <div
      ...
      <div className="flex items-center justify-between gap-3 px-3.5 pt-2.5 pb-1">
        ...
        {onReview && (
          <button
            type="button"
            onClick={onReview}
            className="text-[13px] text-[var(--em-text-secondary)] hover:text-foreground"
          >
            查看
          </button>
        )}
      ...
      <div className="pb-1.5">
        {visible.map((file) => (
          <div
            key={file.key}
            className="group flex items-center gap-2.5 px-3.5 py-2 hover:bg-[var(--em-fill)] transition-colors"
        ...
        {hiddenCount > 0 && (
          <button
            ...
            className="flex w-full items-center gap-2.5 px-3.5 py-2 text-[13px] text-[var(--em-text-secondary)] hover:bg-[var(--em-fill)] hover:text-foreground"
          >

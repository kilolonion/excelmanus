...
import { placeTourCard } from "./tour-layout";
...
export function TourTooltip({ step, stepIndex, totalSteps, phase, phaseLabel, interactionDone, targetRect, onNext, onPrevious, hasPrevious, onSkip, onSkipSection, onSectionChange, onLocate, onPracticeDone, isMobile }: TourTooltipProps) {
  ...
  const position = placeTourCard(targetRect, viewport, isMobile ? 352 : 380, height, step.placement);
  return createPortal(
    <div ref={tooltipRef} className="em-tour-card" role="dialog" aria-modal="false" aria-labelledby="tour-title" aria-describedby="tour-description" data-compact={viewport.height < 460} style={{ ...position, transition: reduceMotion ? "none" : "left 160ms ease, top 160ms ease" }}>
      ...
      <div className="em-tour-progress" role="progressbar" aria-label="本节进度" aria-valuenow={stepIndex + 1} aria-valuemin={0} aria-valuemax={totalSteps}><span style={{ width: `${((stepIndex + 1) / totalSteps) * 100}%` }} /></div>
      <div ref={bodyRef} className="em-tour-body">
        ...
        <div hidden={collapsed}>
          ...
          {step.practice && <TourPractice key={stepId} kind={step.practice} onDone={onPracticeDone} />}

...
_build_frontend_remote() {
  ...
  info "构建前端（npm run build --no-turbopack）..."
  if _remote_frontend "
    export PATH=${NODE_BIN}:\$PATH && \
    cd '${FRONTEND_DIR}/web' && \
  ...
  "; then
    ...
    _remote_frontend "
      cd '${FRONTEND_DIR}/web'
      if [[ ! -f .next/standalone/server.js ]]; then
        echo '[WARN] 构建成功但 standalone/server.js 缺失，请检查 next.config.ts 中 output: standalone 配置'
      fi
      if [[ ! -f .next/BUILD_ID ]]; then
        echo '[WARN] BUILD_ID 缺失，可能仍在使用 Turbopack 构建'
      fi
    " || true
    return 0
  fi

...
7. **收尾动作**：格式化任务结束时，用 `run_code` + openpyxl 自动调整列宽和行高（遍历列计算最大内容宽度设置 `column_dimensions`，按行内容设置 `row_dimensions`），确保表格布局美观。参考 table_layout 策略的相关配置（数字右对齐、文本左对齐、标题居中、单元格居中对齐等）。

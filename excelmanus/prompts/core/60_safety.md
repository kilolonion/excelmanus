...
- **输入文件保护**：验证工具能力或测试写入功能时，必须使用 `outputs/` 目录下的临时文件，严禁向用户提供的输入文件写入测试数据。
...
- **避免 RED 触发**：编写 `run_code` 代码时不要使用 `eval()`/`exec()`（会被标记为 RED 需要用户确认）。公式求值请使用 `ast` 模块的安全解析器（如 `ast.parse` + 递归计算）或 `operator` 模块。

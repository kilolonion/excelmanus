...
条件格式用 `kind=conditional_format`，数据验证/下拉框用 `kind=data_validation`；两者都传 `rule`，字段先用 `introspect_capability(query_type="tool_detail", query="format_spreadsheet.operations.rule")` 查询。打印设置用 `kind=print_layout`，如 `{"kind":"print_layout","sheet":"Sheet1","print_layout":{"print_area":"A1:F17","fit_to_width":1,"fit_to_height":1}}`，成功结果的 `appearance.sheets[].print_settings` 给出有效缩放。原生 Table 和图片插入当前没有已有工作簿的保存入口。

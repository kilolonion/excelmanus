...
本指南提供专业、美观 Excel 表格的设计原则、色板、模板和反模式清单，供格式化/美化任务时按需查阅。可执行字段以当前 `apply_spreadsheet_changes` schema 为准。
...
| X4 | 列宽不统一（忘记 auto_fit） | 文字被截断或大量空白 | 按目标选择显式尺寸或指定轴的 auto_fit；图片复刻保留目标比例 |
...
用户明确要求美化时，可对照上文色板与层级。这不是默认收尾仪式，也不要求固定先 inspect 再 auto_fit。

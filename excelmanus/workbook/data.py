...
_FORMULA_BINOPS: dict[type[ast.operator], Any] = {
...
_FORMULA_UNARYOPS: dict[type[ast.unaryop], Any] = {
    ast.UAdd: operator.pos,
    ast.USub: operator.neg,
    ast.Not: operator.not_,
}
_FORMULA_CMPOPS: dict[type[ast.cmpop], Any] = {
    ast.Eq: operator.eq,
...
def _eval_formula_ast(expr: str, namespace: dict[str, Any]) -> Any:
    """对公式表达式做 AST 白名单求值，禁止 Call/Attribute 等任意代码执行。"""
    ...
    return _eval_formula_node(tree.body, namespace)


def _eval_formula_node(node: ast.AST, namespace: dict[str, Any]) -> Any:
    if isinstance(node, ast.Constant):
        if isinstance(node.value, (int, float, complex)) or node.value is None:
            return node.value
        raise ValueError(f"disallowed constant: {type(node.value).__name__}")
    if isinstance(node, ast.Name):
        return namespace[node.id]
    if isinstance(node, ast.UnaryOp):
        fn = _FORMULA_UNARYOPS.get(type(node.op))
        if fn is None:
            raise ValueError(f"disallowed unary op: {type(node.op).__name__}")
        return fn(_eval_formula_node(node.operand, namespace))
    if isinstance(node, ast.BinOp):
        fn = _FORMULA_BINOPS.get(type(node.op))
        if fn is None:
            raise ValueError(f"disallowed binop: {type(node.op).__name__}")
        return fn(
            _eval_formula_node(node.left, namespace),
            _eval_formula_node(node.right, namespace),
        )
    if isinstance(node, ast.BoolOp):
        values = [_eval_formula_node(v, namespace) for v in node.values]
        result = values[0]
    ...
    if isinstance(node, ast.Compare):
        current = _eval_formula_node(node.left, namespace)
        acc: Any = None
        for op, comparator in zip(node.ops, node.comparators, strict=True):
            fn = _FORMULA_CMPOPS.get(type(op))
            if fn is None:
                raise ValueError(f"disallowed compare op: {type(op).__name__}")
            nxt = _eval_formula_node(comparator, namespace)
            cmp_val = fn(current, nxt)
            acc = cmp_val if acc is None else acc and cmp_val
            current = nxt
        return acc
    if isinstance(node, ast.Expression):
        return _eval_formula_node(node.body, namespace)
    if isinstance(node, ast.Expr):
        return _eval_formula_node(node.value, namespace)
    if isinstance(node, ast.Module) and len(node.body) == 1:
        return _eval_formula_node(node.body[0], namespace)
    raise ValueError(f"disallowed AST node: {type(node).__name__}")
...
def _resolve_formula_columns(
    ...
    try:
        ...
        for col_idx in nan_cols_idx:
            ...
            try:
                result = _eval_formula_ast(expr, namespace)
    ...
    *,
    ...
    if summary.get("error") and "shape" not in summary and "columns" not in summary:
        ...
        return ToolResult(
            success=False,
            model_text=message,
            error=ToolError(code="EXECUTION_FAILED", message=message, fields=summary),
        )
    ...
    lines = [
        f"文件 {summary.get('file', rel_path)}{sheet_part}：{rows} 行 × {cols} 列。",
    ]
    if col_preview:
    ...
    if content_version is None and file_path:
        from pathlib import Path

        from excelmanus.workbook_commit import content_version_of_file

        content_version = content_version_of_file(Path(file_path))
    if content_version:
    ...
    *,
    ...
    if result.get("error"):
        ...
        return ToolResult(
            success=False,
            model_text=message,
            error=ToolError(code="EXECUTION_FAILED", message=message, fields=result),
        )
    ...
    if truncated:
        lines.append("⚠️ 差异样本已截断，完整 diff 见前端展示。")
    if sample_diffs:
        lines.append(
            f"样本（前 {min(3, len(sample_diffs))} 条）："
...
def _file_content_version(file_path: Any) -> str | None:
    ...
    return content_version_of_file(Path(file_path))
...
def _finalize_filter_data_result(
    ...
    *,
    ...
    content_version = _file_content_version(file_path)
...
def _finalize_scan_excel_snapshot_result(
    ...
    *,
    ...
    lines = [
        f"文件 {result.get('file', rel_path)}（{result.get('size', '')}）："
        f"{result.get('sheet_count', len(sheets))} 个工作表。",
    ]
    ...
    if signals:
        types = [str(s.get("type")) for s in signals[:6] if isinstance(s, dict)]
        lines.append(f"质量信号 {len(signals)} 条：{', '.join(types)}。")
...
def _finalize_search_excel_values_result(
    ...
    *,
    ...
    if file_path is not None and len(rel_paths) == 1:
        content_version = _file_content_version(file_path)
    if content_version:
...
def _read_range_direct(
    ...
    *,
    ...
    if None in bounds:
        raise ValueError(f"range 不支持整列/整行地址 {cell_range!r}，请写 A1:F20")
    ...
    payload: dict[str, Any] = {
        ...
        "data": rows,
        "resolved_sheet": resolved_title,
        "formulas_uncached": True,
    }

    if include_formulas:
        wb_f = load_workbook(safe_path, read_only=True, data_only=False)
        try:
            ws_f = wb_f[resolved_title] if resolved_title in wb_f.sheetnames else wb_f.active
            ...
            payload["formula_grid"] = formula_rows
            payload["formulas_uncached"] = False
        finally:
            wb_f.close()
    return payload
...
def read_excel(
    ...
    rel_path = _workspace_rel_path(guard, safe_path, file_path)
    ...
    bound_version = content_version_of(safe_path.read_bytes())
    ...
    if range is not None and (max_rows is not None or offset is not None or sample_rows is not None):
        ...
        return _error_payload_result(
            {
                "error": "精确 range 不能同时使用 max_rows/offset/sample_rows",
                "code": "INVALID_ARGS",
    ...
    def _finish_read(summary: dict[str, Any], **kwargs: Any) -> ToolResult:
        if content_version_of_file(safe_path) != bound_version:
            return ToolResult(
                success=False,
                model_text="文件在读取过程中被修改，请重试",
                error=ToolError(
                    code="STALE_READ",
                    message="文件在读取过程中被修改，请重试",
                ),
            )
    ...
    if not _is_csv_file(safe_path):
        if sheet_name is not None:
            ...
            from openpyxl import load_workbook as _lw_default

            _wb_default = _lw_default(safe_path, read_only=True)
            try:
                sheet_name = _wb_default.sheetnames[0] if _wb_default.sheetnames else None
            finally:
                _wb_default.close()
    ...
    if range is not None and not _is_csv_file(safe_path):
        ...
        try:
            ...
            return _error_payload_result(
                {
                    "error": (
                        f"range={range!r} 不是合法坐标：{exc}。"
                        "请用 A1:F20，或 表名!A1:F20。"
                    ),
                    "code": "INVALID_ARGS",
                },
                code="INVALID_ARGS",
...
def filter_data(
    ...
    # .xls/.xlsb → 透明转换为 xlsx
    from excelmanus.tools._helpers import check_sheet_name, ensure_openpyxl_compatible
    safe_path = ensure_openpyxl_compatible(safe_path)
    ...
    if not _is_csv_file(safe_path):
        if sheet_name is not None:
            ...
            from openpyxl import load_workbook as _lw_filter

            _wb_filter = _lw_filter(safe_path, read_only=True)
            try:
                sheet_name = _wb_filter.sheetnames[0] if _wb_filter.sheetnames else None
            finally:
                _wb_filter.close()

    df, _ = _read_df(safe_path, sheet_name, header_row=header_row)

    def _contains_literal(s: pd.Series, v: Any) -> pd.Series:
        return s.astype(str).str.contains(str(v), na=False, regex=False)

    def _contains_regex(s: pd.Series, v: Any) -> pd.Series:
        return s.astype(str).str.contains(str(v), na=False, regex=True)

    def _between(s: pd.Series, v: Any) -> pd.Series:
        if not isinstance(v, (list, tuple)) or len(v) < 2:
            raise ValueError("between 需要长度为 2 的数组，如 [min, max]")
        return s.between(v[0], v[1])
    ...
    ops = {
        ...
        "lt": lambda s, v: s < v,
        "le": lambda s, v: s <= v,
        "contains": _contains_literal,
        "regex": _contains_regex,
    ...
    for cond in cond_list:
        ...
        if isinstance(op, str):
            key = op.strip()
            op = _op_aliases.get(key) or _op_aliases.get(key.lower()) or key.lower()
        ...
        try:
            ...
            return _error_payload_result({"error": str(exc), "code": "INVALID_ARGS"}, code="INVALID_ARGS")
...
def compare_excel(
    ...
    rel_a = _workspace_rel_path(guard, safe_a, file_a)
    rel_b = _workspace_rel_path(guard, safe_b, file_b)
    ...
    if cells_different == 0 and rows_added == 0 and rows_deleted == 0 and not structure_changed:
        ...
        result["hint"] = (
            f"共发现 {'、'.join(parts)}。完整 diff 已通过前端展示。"
            if parts
            else "结构或数据存在差异。"
        )
...
def scan_excel_snapshot(
    ...
    if _is_csv_file(safe_path):
        return _scan_csv_snapshot(
            safe_path, size_str, max_sample_rows, rel_path=rel_path,
        )
    ...
    return _finalize_scan_excel_snapshot_result(
        result, rel_path=rel_path, file_path=safe_path,
    )
...
def search_excel_values(
    ...
    return _finalize_search_excel_values_result(
        result, rel_paths=[rel_path], file_path=safe_path,
    )

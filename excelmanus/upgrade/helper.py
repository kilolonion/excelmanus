...
def api_is_running(port: int | None = None) -> bool:
    if port is None:
        ...
        port = int(raw) if raw is not None else int(os.environ.get("EXCELMANUS_API_PORT", "8000"))
...
def _kill_port(port: int) -> None:
        ...
        try:
            ...
            for line in r.stdout.splitlines():
                if f":{port}" in line and "LISTENING" in line.upper():
                    parts = line.split()
                    if parts:
                        _kill_pid(int(parts[-1]), force=True)
    ...
    try:
        ...
        for token in r.stdout.split():
            try:
                _kill_pid(int(token), force=True)
            except ValueError:
                continue
...
def stop_supervised(runtime: dict[str, Any] | None, *, wait_s: float = 20.0) -> None:
    """停掉 start.sh/ps1 监督的进程组，并清理端口。

    监督进程已不在则不 killpg（避免陈旧 runtime.json 误杀无关进程组），只按端口清理。
    """
    runtime = runtime or {}
    me = os.getpid()
    ...
    if not ports:
        ports = [
            int(os.environ.get("EXCELMANUS_API_PORT", "8000")),
            int(os.environ.get("EXCELMANUS_FRONTEND_PORT", "3000")),
        ]
    for port in ports:
        if _port_busy(port):
            _kill_port(port)
...
def spawn_detached_helper(project_root: Path) -> None:
    ...
    log_path = Path(os.environ.get("TMPDIR") or os.environ.get("TEMP") or "/tmp") / "excelmanus-upgrade.log"
